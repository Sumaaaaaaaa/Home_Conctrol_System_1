...
# 【方法】
# Log对象.Record(content: str, path:str="?Unknown?", isError: bool = False)
...
class Log(object):
    ...
    def Record(self, content: str, path: str = "?Unknown?", isError: bool = False):
    ...
    def __Console_Print(self, content, path, isError):
        if path == "?Unknown?": path = "\033[31m?Unknown?\033[0m"
        if isError:
            print(
